...
class Logger(object):
    def __init__(self):
        ...
        self.log = open("console_Rho2.log", "a")
...
def generateSyntheticData(G1):
    ...
    plt.savefig("plots_rho2/Original_Grouping.png")
    plt.clf()
    
    plt.imshow(a_true_nodes, aspect='auto', interpolation='none', origin='lower')
    plt.colorbar()
    plt.savefig("plots_rho2/Original_Grouping_nodes.png")
    plt.clf()
    ...
    plt.colorbar()
    plt.savefig("plots_rho2/Original_Grouping_nodes_Similarity.png")
    plt.clf()
...
def runExperiments(G1, lamb, rho, trainingSet, x_test, y_test, a_true, maxProcesses, eabs, erel, admmMaxIteration, directory):
    ...
    print "\t For Rho = ", rho
    start_time = time.time()
    (x, u, z, iters) = runADMM(G1, lamb, rho, x, u , z, trainingSet, A, sqn, sqp, maxProcesses, eabs, erel, admmMaxIteration, directory, x_train, y_train, x_test, y_test)
...
def main():
    ...
    while(rho>.00001):
        ...
        if not os.path.exists("plots_rho2/Accuracies"):
            os.makedirs("plots_rho2/Accuracies")
        if not os.path.exists("plots_rho2/Time"):
            os.makedirs("plots_rho2/Time")
        if not os.path.exists("plots_rho2/Iterations"):
            os.makedirs("plots_rho2/Iterations")
        ...
        plt.legend()
        plt.savefig("plots_rho2/Accuracies/"+ str(rho) +".png")
        plt.clf()
        
        plt.plot(pl5, pl4)
        plt.xlabel('rho')
        plt.ylabel('Time (minutes)')
        plt.savefig("plots_rho2/Time/"+ str(rho) +".png")
        plt.clf()
        
        plt.plot(pl5, pl3)
        plt.xlabel('rho')
        plt.ylabel('Iterations')
        plt.savefig("plots_rho2/Iterations/"+ str(rho) +".png")
        plt.clf()
