...
#     plt.yticks(np.arange(0, a_true.shape[0], dtype=np.int))
#     plt.show()
    ...
    a_train = np.zeros((sizeOptimizationVariable,nodes))
...
#     plt.imshow(a_train, aspect='auto', interpolation='none', origin='lower')
#     plt.colorbar()
#     plt.show()
#     similarities = cosine_similarity(a_train.transpose())
...
def getAccuracy(modelParameters, dataSetSizeTrain, featureData, labelData,set):
    write = np.zeros((nodes, 6))
    (right, total) = (0, dataSetSizeTrain * nodes)
    ...
    for i in range(nodes):
        ...
        write[i][0] = i +1
        nodeRight = 0
        for j in range(dataSetSizeTrain):
        ...
        write[i][5] = nodeRight/float(dataSetSizeTrain)
    ...
    if(set == 1):       
        np.savetxt("training.csv",write,  fmt='%.2f', delimiter=",")
    else:
        np.savetxt("testing.csv",write,  fmt='%.2f', delimiter=",")
...
#             
        # Get accuracy
        print "\t \t Result"         
        print "\t \t \t Train Data Accuracy = ", getAccuracy(x, trainSetSize, x_train, y_train,1)
        testAccuracy = getAccuracy(x, testSetSize, x_test, y_test,2)
