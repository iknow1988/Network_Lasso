...
def generateSyntheticData(G1):
    ...
    return trainingSet, x_test, y_test, a_true, a_true_nodes
...
def runADMM(G1, lamb, rho, x, u, z, a, A, sqn, sqp, maxProcesses, eabs, erel, admmMaxIteration, directory, x_train, y_train, x_test, y_test, a_true_nodes):
    ...
    while(r > epri or s > edual):
        ...
        similarities = np.absolute(x.transpose() - a_true_nodes.transpose())
        plt.imshow(similarities.transpose(), aspect='auto', interpolation='none', origin='lower', cmap='Greys')
        plt.colorbar()
        plt.savefig(directory+"/" + str(iters) +".png")
        plt.clf()
...
def runExperiments(G1, lamb, rho, trainingSet, x_test, y_test, a_true, maxProcesses, eabs, erel, admmMaxIteration, directory, a_true_nodes):
    ...
    plt.imshow(similarities.transpose(), aspect='auto', interpolation='none', origin='lower', cmap='Greys')
    plt.colorbar()
    plt.savefig("Plots/Similarities/"+str(lamb)+".png")
    plt.clf()
