...
class Logger(object):
    def __init__(self):
        ...
        self.log = open("console_Stopping.log", "a")
...
def generateSyntheticData(G1):
    ...
    plt.savefig("PlotsStopping/Original_Grouping.png")
    plt.clf()
    
    plt.imshow(a_true_nodes, aspect='auto', interpolation='none', origin='lower')
    plt.colorbar()
    plt.savefig("PlotsStopping/Original_Grouping_nodes.png")
    plt.clf()
    ...
    plt.colorbar()
    plt.savefig("PlotsStopping/Original_Grouping_nodes_Similarity.png")
    plt.clf()
...
def runExperiments(G1, lamb, rho, trainingSet, x_test, y_test, a_true, maxProcesses, eabs, erel, admmMaxIteration, directory):
    ...
    # run ADMM
    print "\t For Stopping criterion = ", eabs
    start_time = time.time()
...
def main():
    ...
    while(eabs>.00001):
        ...
        erel = eabs
        lamb = 1.2
        directory = "PlotsStopping/" + str(eabs)
        ...
        if not os.path.exists("PlotsStopping/Accuracies"):
            os.makedirs("PlotsStopping/Accuracies")
        if not os.path.exists("PlotsStopping/Time"):
            os.makedirs("PlotsStopping/Time")
        if not os.path.exists("PlotsStopping/Iterations"):
            os.makedirs("PlotsStopping/Iterations")
        ...
        plt.legend()
        plt.savefig("PlotsStopping/Accuracies/"+ str(eabs) +".png")
        plt.clf()
        
        plt.plot(pl5, pl4)
        plt.xlabel('Stopping')
        plt.ylabel('Time (minutes)')
        plt.savefig("PlotsStopping/Time/"+ str(eabs) +".png")
        plt.clf()
        
        plt.plot(pl5, pl3)
        plt.xlabel('Stopping')
        plt.ylabel('Iterations')
        plt.savefig("PlotsStopping/Iterations/"+ str(eabs) +".png")
        plt.clf()
